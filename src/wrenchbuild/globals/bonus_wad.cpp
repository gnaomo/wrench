...
packed_struct(UyaJapanKoreaBonusWadHeader,
	/* 0x000 */ s32 header_size;
	/* 0x004 */ Sector32 sector;
	/* 0x008 */ u64 unused_8[183];
...
on_load(Bonus, []() {
	...
	BonusWadAsset::funcs.unpack_rac3 = wrap_wad_unpacker_func_2<BonusWadAsset, UyaBonusWadHeader, UyaJapanKoreaBonusWadHeader>(
		unpack_uya_dl_bonus_wad<UyaBonusWadHeader>, unpack_uya_dl_bonus_wad<UyaJapanKoreaBonusWadHeader>);
	...
	BonusWadAsset::funcs.pack_rac3 = wrap_wad_packer_func_2<BonusWadAsset, UyaBonusWadHeader, UyaJapanKoreaBonusWadHeader>(
		pack_uya_dl_bonus_wad<UyaBonusWadHeader>, pack_uya_dl_bonus_wad<UyaJapanKoreaBonusWadHeader>);
...
bool pack_uya_dl_bonus_wad(
	OutputStream& dest, Header& header, const BonusWadAsset& src, BuildConfig config)
{
	// The Japanese/Korean version of UYA has a different header. First the packer will
	// try to call the version of this function with the normal header, so bail
	// out so it will try the version with the Japan/Korea header.
	if (config.game() == Game::UYA && (config.region() == Region::JAPAN || config.region() == Region::KOREA) && std::is_same_v<Header, UyaBonusWadHeader>) {
		return false;
	}
